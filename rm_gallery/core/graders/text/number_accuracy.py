...
by comparing numbers extracted from the response text with those in a reference text.
...
class NumberAccuracyGrader(BaseGrader):
    """
    Check numerical calculation accuracy by comparing numbers in response vs reference content.
    ...
    Methods:
        evaluate: Extracts and compares numbers between response and reference content.
    ...
    def __init__(self, tolerance: float = 1e-6, **kwargs: Any) -> None:
        ...
        super().__init__(
            ...
            description="Check numerical calculation accuracy by comparing numbers in response vs reference content",
            **kwargs,
        )
    ...
    async def aevaluate(self, response: str, reference: str) -> GraderScore:
        ...
        This method extracts numerical values from both the response and reference texts,
        then compares them in order to compute an accuracy score. The score represents the
        proportion of numbers in the reference that were correctly reproduced in the response text.
        ...
        correct = 0
        total = min(len(response_numbers), len(reference_numbers))

        for i in range(total):
            if abs(response_numbers[i] - reference_numbers[i]) <= self.tolerance:
                correct += 1
